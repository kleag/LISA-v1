...
class Dataset(Configurable):
    ...
    def __init__(self, filename, vocabs, builder, *args, **kwargs):
        ...
        self.train_domains_set = (set(self.train_domains.split(','))
                                  if (self.train_domains != '-'
                                      and self.name == "Trainset")
                                  else set())
        ...
        self.targets = tf.compat.v1.placeholder(dtype=tf.int32,
                                                shape=(None, None, None),
                                                name='targets')
        self.step = tf.compat.v1.placeholder_with_default(0., shape=None,
                                                          name='step')
    ...
    def _process_buff(self, buff):
        ...
        for i, sent in enumerate(buff):
            ...
            srl_take_indices = [
              ...
              if idx < num_fields - 1 and (
                self.train_on_nested or np.all(
                ...
                if self.conll:
                    ...
                    if self.joint_pos_predicates:
                        is_predicate = (token[predicates.conll_idx[0]] != '-'
                                        and (self.train_on_nested
                                             or self.predicate_str
                                             in srl_fields))
                        tok_predicate_str = str(is_predicate) + '/' + gold_tag
                    else:
                        is_predicate = (token[predicates.conll_idx] != '-'
                                        and (self.train_on_nested
                                             or self.predicate_str
                                             in srl_fields))
            ...
            if self.one_example_per_predicate:
                ...
                if predicate_indices:
                    for k, p_idx in enumerate(predicate_indices):
                        # should be sent_len x sent_elements
                        rest_part[:, is_predicate_idx-1] = predicates[
                          "False"][0]
                        rest_part[p_idx, is_predicate_idx-1] = predicates[
                          "True"][0]
                        correct_srls = srl_part[:, k]
                        new_sent = np.concatenate(
                          [np.expand_dims(word_part, -1), rest_part,
                           np.expand_dims(correct_srls, -1)], axis=1)
                    ...
                    new_sent = np.concatenate([np.expand_dims(word_part, -1),
                                              rest_part], axis=1)
                    buff2.append(new_sent)
                    examples += 1
            # else:
    ...
    # =============================================================
    def get_minibatches(self, batch_size, input_idxs, target_idxs,
                        shuffle=True):
        """"""
    ...
    def max_batch_size(self):
        ...
        max_batch_size = np.max([b._data.shape[0]
                                 for b in self._metabucket._buckets])
