...
class NN(Configurable):
  ...
  def output_svd(self, logits3D, targets3D):
    ...
    def compute_loss(logits3D, tokens_to_keep1D):
      ...
      log_loss = tf.Print(log_loss, [log_loss, tf.reduce_sum(cross_entropy1D * tokens_to_keep1D), tf.reduce_sum(cross_entropy1D), tf.reduce_sum(logits2D), targets1D],
                          "sums", summarize=20)
    ...
    log_loss = tf.Print(log_loss, [log_loss, tf.greater(tf.rank(logits3D), 1), self.n_tokens], "log loss")
