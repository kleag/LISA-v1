...
def layer_norm(inputs, reuse, epsilon=1e-6):
  ...
  """
  with tf.compat.v1.variable_scope("layer_norm", reuse=reuse):
...
def dot_product_attention(q, k, v,
  ...
  Returns:
    A Tensor.
  """
  with tf.compat.v1.variable_scope(name, default_name="dot_product_attention", values=[q, k, v]):
    ...
    weights_drop = tf.nn.dropout(weights, rate=(1-dropout_rate))
...
def compute_qkv(antecedent, total_key_depth, total_value_depth):
  ...
  """
  params = tf.compat.v1.get_variable("qkv_transform", [1, 1, total_key_depth, 2*total_key_depth + total_value_depth])
...
def multihead_attention(antecedent,
  ...
  with tf.compat.v1.variable_scope(name, default_name="multihead_attention", values=[antecedent]):
    ...
    x = combine_heads(x)
    params = tf.compat.v1.get_variable("final_proj", [1, 1, total_key_depth, output_depth])
...
def conv_hidden_relu(inputs,
  ...
  with tf.compat.v1.variable_scope("conv_hidden_relu", [inputs]):
    inputs = tf.expand_dims(inputs, 1)
    in_size = inputs.get_shape().as_list()[-1]
    params1 = tf.compat.v1.get_variable("ff1", [1, 1, in_size, hidden_size])
    params2 = tf.compat.v1.get_variable("ff2", [1, kernel, hidden_size, hidden_size])
    params3 = tf.compat.v1.get_variable("ff3", [1, 1, hidden_size, output_size])
...
class NN(Configurable):
  ...
  def embed_concat(self, word_inputs, tag_inputs=None, rel_inputs=None):
    ...
    if self.moving_params is None:
      ...
      if word_keep_prob < 1:
        word_mask = tf.nn.dropout(tf.ones(noise_shape), rate=(1-word_keep_prob))*word_keep_prob
      else:
        word_mask = 1
      if tag_inputs is not None and tag_keep_prob < 1:
        tag_mask = tf.nn.dropout(tf.ones(noise_shape), rate=(1-tag_keep_prob))*tag_keep_prob
      else:
        tag_mask = 1
      if rel_inputs is not None and rel_keep_prob < 1:
        rel_mask = tf.nn.dropout(tf.ones(noise_shape), rate=(1-rel_keep_prob))*rel_keep_prob
      else:
        rel_mask = 1
  ...
  def RNN(self, inputs):
    ...
    if self.recur_bidir:
      ...
      top_recur.set_shape([tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(2*self.recur_size)])
    else:
      top_recur, end_recur = rnn.dynamic_rnn(cell, inputs,
                                             ...
                                             recur_keep_prob=recur_keep_prob,
                                             dtype=tf.float32)
      top_recur.set_shape([tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(self.recur_size)])
  ...
  def CNN(self, inputs, kernel1, kernel2, output_size, dropout_keep_rate, nonlinearity):
    ...
    params = tf.compat.v1.get_variable('CNN', [kernel1, kernel2, input_size, output_size], initializer=initializer)
    if kernel1 == 1:
      inputs = tf.expand_dims(inputs, 1)
    ...
    conv_out = tf.nn.dropout(conv_out, rate=(1-dropout_keep_rate))
    return conv_out
  ...
  def transformer(self, inputs, hidden_size, num_heads, attn_dropout, relu_dropout, prepost_dropout, relu_hidden_size,
    ...
    with tf.compat.v1.variable_scope("self_attention"):
      ...
      x = tf.add(x, tf.nn.dropout(y, rate=(1-prepost_dropout)))
  ...
  def soft_attn(self, top_recur):
    ...
    top_recur.set_shape([tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(4*self.recur_size)])
    return top_recur
  ...
  def linear(self, inputs, output_size, n_splits=1, add_bias=False):
    ...
    if keep_prob < 1:
      ...
      inputs = tf.nn.dropout(inputs, rate=(1-keep_prob), noise_shape=noise_shape)
  ...
  def double_MLP(self, inputs, n_splits=1):
    ...
    shape_to_set = [tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(output_size)]
  ...
  def bilinear_classifier(self, inputs1, inputs2, add_bias1=True, add_bias2=False, keep_prob=None):
    ...
    if isinstance(keep_prob, tf.Tensor) or keep_prob < 1:
      noise_shape = tf.stack([batch_size, 1, input_size])
      # Experimental
      #inputs1 = tf.nn.dropout(inputs1, rate=(1-keep_prob) if add_bias2 else tf.sqrt(keep_prob), noise_shape=noise_shape)
      #inputs2 = tf.nn.dropout(inputs2, rate=(1-keep_prob) if add_bias1 else tf.sqrt(keep_prob), noise_shape=noise_shape)
      inputs1 = tf.nn.dropout(inputs1, rate=(1-keep_prob), noise_shape=noise_shape)
      inputs2 = tf.nn.dropout(inputs2, rate=(1-keep_prob), noise_shape=noise_shape)
  ...
  def diagonal_bilinear_classifier(self, inputs1, inputs2, add_bias1=True, add_bias2=False):
    ...
    if isinstance(keep_prob, tf.Tensor) or keep_prob < 1:
      noise_shape = tf.stack([batch_size, 1, input_size])
      inputs1 = tf.nn.dropout(inputs1, rate=(1-tf.sqrt(keep_prob)), noise_shape=noise_shape)
      inputs2 = tf.nn.dropout(inputs2, rate=(1-tf.sqrt(keep_prob)), noise_shape=noise_shape)
  ...
  def bilinear_classifier_nary(self, inputs1, inputs2, n_classes, add_bias1=True, add_bias2=True):
    ...
    # with tf.control_dependencies([tf.compat.v1.assert_equal(input_shape1[1], input_shape2[1])]):
    ...
    input_shape_to_set1 = [tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(None), input_size1 + 1]
    input_shape_to_set2 = [tf.compat.v1.Dimension(None), tf.compat.v1.Dimension(None), input_size2 + 1]
    ...
    if isinstance(keep_prob, tf.Tensor) or keep_prob < 1:
      ...
      inputs1 = tf.nn.dropout(inputs1, rate=(1-keep_prob), noise_shape=noise_shape1)
      inputs2 = tf.nn.dropout(inputs2, rate=(1-keep_prob), noise_shape=noise_shape2)
    ...
    def compute_srl_loss(logits_transposed, srl_targets_transposed, transition_params):
      ...
      if transition_params is not None:
        ...
        if self.label_smoothing > 0:
          ...
          loss = tf.compat.v1.losses.softmax_cross_entropy(logits=tf.reshape(logits_transposed, [-1, num_labels]),
  ...
  def logits_mask_roots(self, logits3D, batch_size, bucket_size):
    ...
    roots_logits = tf.linalg.diag_part(logits3D)  # doing this again with pairs mask applied
    ...
    roots_mask = tf.linalg.diag(diagonal_masked)
  ...
  def gen_targets_mask(self, targets3D, batch_size, bucket_size):
    ...
    with tf.control_dependencies(
            [tf.compat.v1.assert_equal(tf.reduce_sum(tf.linalg.diag_part(targets_mask), axis=1), tf.ones([batch_size]))]):
      return targets_mask
  ...
  def compute_svd_loss(self, logits2D, tokens_to_keep1D, batch_size, bucket_size):
    ...
    # zero out diagonal
    adj = tf.linalg.set_diag(adj, tf.zeros([batch_size, bucket_size]))
    # make it undirected
    ...
    laplacian = tf.linalg.set_diag(-undirected_adj, degrees)
  ...
  def compute_roots_loss(self, logits3D, targets_mask):
    ...
    roots_logits = tf.linalg.diag_part(logits3D)
    roots_targets1D = tf.argmax(tf.linalg.diag_part(targets_mask), axis=1)
