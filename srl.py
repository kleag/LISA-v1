...
class Network(Configurable):
    ...
    def __init__(self, model, *args, **kwargs):
        ...
        if self.conll:
            vocab_files = [(self.word_file, 1, 'Words', self.embed_size),
                           (self.tag_file, [3, 4], 'Tags',
                            self.embed_size if self.add_pos_to_input else 0),
                           (self.rel_file, 7, 'Rels', 0)]
        elif self.conll2012:
            vocab_files = [(self.word_file, 3, 'Words', self.embed_size),
                           (self.tag_file, [5, 4], 'Tags',
                            # auto, gold
                            self.embed_size if self.add_pos_to_input else 0),
                           (self.rel_file, 7, 'Rels', 0),
                           (self.srl_file, list(range(14, 50)), 'SRLs', 0),
                           (self.predicates_file,
                            [10, 4] if self.joint_pos_predicates else 10,
                            'Predicates',
                            (self.predicate_embed_size
                             if self.add_predicates_to_input else 0)),
        ...
        # print("Loading data")
        self._trainset = Dataset(None, self._vocabs, model, self._config,
                                 name='Trainset')

        self._ops = self._gen_ops()
        self._save_vars = [x for x in tf.compat.v1.global_variables()
                           if 'Pretrained' not in x.name]
        return
    ...
    def analyze(self, sess, text):
        ...
        print(f"Network.analyze writing to temp file {temp.name}:",
              file=sys.stderr)
        ...
        for sentence_id, sentence in enumerate(doc.sents):
            ...
            for token_id, token in enumerate(sentence):
                if (len(token.text) > 0 and token.text != '\n'
                    ...
                    # associate token pos and len to the identification of the
                    # token
                    tokens_localization[(sentence_id, real_token_id)] = (
                      token.idx, token.__len__())
                    real_token_id += 1
                    ...
                    if token.dep_ == "ROOT":
                        ...
                        dep = (dep_transcat_table[token.dep_]
                               if token.dep_ in dep_transcat_table
                               else token.dep_)
        ...
        for batch_num, (feed_dict, sents) in enumerate(analyze_minibatches()):
            ...
            print(f"Network.analyze model finished handling a batch",
                  file=sys.stderr)
            preds, _, _, _, _, _, _, _, _ = self.model.validate(
              mb_inputs, mb_targets, probs, n_cycles, len_2_cycles, srl_preds,
              srl_logits, srl_predicates, srl_predicate_targets, pos_preds,
              None)
        ...
        for p_idx, (bkt_idx, idx) in enumerate(data_indices):
            preds = (all_predictions[p_idx] if self.one_example_per_predicate
                     else all_predictions[bkt_idx][idx])
            sentence_id += 1
            ...
            for i, (word, pred) in enumerate(zip(words, preds)):
                ...
                try:
                    position, length = tokens_localization[
                      (sentence_id-1, tok_id-1)]
                    tup = (
                          ...
                          # pred tag or auto tag
                          # self.tags[pred[11]] if self.joint_pos_predicates
                          # or self.train_pos else self.tags[pred[4]],
                          "_",
                          "_",
                    ...
                    print(f"There is no such token localization "
                          f"({sentence_id-1}, {tok_id-1})",
                          file=sys.stderr)
            result += "\n"
        return result
    ...
    def _gen_ops(self):
        """
        Generate a graph for train set and return a dict with all tensors
        and the output values necessary to compute the result
        """

        self._optimizer = RadamOptimizer(self._config,
...
class Analyzer(Configurable):
    ...
    def __init__(self, cargs):
        ...
        # config_proto.gpu_options.per_process_gpu_memory_fraction =
        # self._network.per_process_gpu_memory_fraction
        ...
        print(f"Analyzer() Loading model: {self._network.load_dir}",
              file=sys.stderr)
