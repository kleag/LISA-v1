...
for lr in ${lrs[@]}; do
    for mu in ${mus[@]}; do
                                    ...
                                    for head_size in ${head_sizes[@]}; do
                                        for relu_hidden_size in ${relu_hidden_sizes[@]}; do
                                            for batch_size in ${batch_sizes[@]}; do
                                                for ff_kernel in ${ff_kernels[@]}; do
        ...
        #                                                                    if [[ "$cnn_layer" != "2" || "$trans_layer" != "10" ]]; then
                                                                                fname_append="$rep-$lr-$mu-$nu-$epsilon-$warmup_steps-$batch_size-$cnn_dim-$cnn_layer-$trans_layer-$num_head-$head_size-$relu_hidden_size-$ff_kernel-$parents_penalty-$parents_layer-$predicate_layer-$ss-$full_parse"
